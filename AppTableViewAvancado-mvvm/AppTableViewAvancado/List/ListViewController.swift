...
extension ListViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row != 0 && indexPath.row != 1 {
            let employee = viewModel.loadCurrentEmployee(row: indexPath.row - 2)
            print(employee.name)
        }
    }
}
...
extension ListViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == 0 || indexPath.row == 1 {
            ...
            cell?.delegate = self
            cell?.setupCell(employee: viewModel.loadCurrentEmployee(row: indexPath.row - 2))
            return cell ?? UITableViewCell()
        }
    }
}
...
extension ListViewController: EmployeeTableViewCellProtocol {
    func tappedUpdateHeartButton(employee: Employee) {
        viewModel.tappedUpdateHeartButton(employee: employee)
    }
}
